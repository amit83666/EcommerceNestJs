...
export class PasswordresetController {
  constructor(
    ...
    async requestPasswordReset(@Body() body: {email:string}): Promise<void>{
      ...
      await this.passwordresetService.createToken(user.userId, token, expiresAt);
    }
    @Post('reset/:token')
    @Public()
    async resetPassword(@Param('token') token:string, @Body() resetPasswordDto: ResetPasswordDto):Promise<void>{
      ...
      if(!passwordReset || this.tokenService.isTokenExpired(passwordReset.expiresAt)){
        throw new NotFoundException('Invalid or expired token.');
      }
      const user = await this.userService.findById(passwordReset.id);
      if(!user){
